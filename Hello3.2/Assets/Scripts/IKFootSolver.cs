...
public class IKFootSolver : MonoBehaviour
{
    ...
    float oldRootPositionY, currentRootPositionY, newRootPositionY;
    ...
    private void Start()
    {
        root = transform.root.GetComponent<Transform>();
        ...
        oldRootPositionY = currentRootPositionY = newRootPositionY = transform.root.position.y;
    ...
    private void Update()
    {
        ...
        */
        ...
        if(temp.localRotation.eulerAngles.y != 0 )
            temp.localRotation = Quaternion.Euler(temp.localRotation.eulerAngles.x, 0, temp.localRotation.eulerAngles.z);
        ...
        if (Physics.Raycast(ray, out RaycastHit info, 5, groundLayer.value))
        {
            ...
            if (Vector3.Distance(currentPosition, info.point) > stepDistance && lerp >= 1 && !otherFoot.IsMoving())  //
            {
                ...
                newPosition = info2.point + footOffset;
                //newRootPositionY = info2.point.y;
                //info2.normal = Quaternion.Euler(0, 90, 0) * info2.normal;
                //info2.normal = Quaternion.AngleAxis(-90f, Vector3.up) * info2.normal;
        ...
        if (lerp < 1)
        {
            ...
            /*
            float tempRootPositionY = Mathf.Lerp(oldRootPositionY, newRootPositionY, lerp);
            tempRootPositionY -= Mathf.Sin(lerp * Mathf.PI) * raise.y;
            currentRootPositionY = tempRootPositionY;
            */
        ...
        {
            oldRootPositionY = newRootPositionY;
